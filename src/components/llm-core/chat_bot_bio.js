 ...
 * ADAPTED FOR GEMINI API RESPONSE
 */
import llmQuery from './llm_querry.js';
import { eukaryoticCellText, eukaryoticCellTopicsExp } from '../content/eukaryoticCellText.js';
...
class ChatBot {
  ...
  async callStudentLLM() {
    ...
    // Gemini API returns a raw string.
    return llmQuery(messages).then((resultText) => {
      const splitMsg = resultText.split("[SMILE]");
      ...
      const m = { role: 'assistant', content: resultText };
      this.addToMsgList(m);
      return splitMsg;
    })
    .catch((error) => {
  ...
  async callTASMisconception() {
    ...
    // Gemini API returns a raw string.
    return llmQuery(messages).then((resultText) => {
      return detectedMisconception(resultText);
    })
    .catch((error) => {
      console.error("Error:", error);
      return false;
